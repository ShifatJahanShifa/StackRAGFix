...
async function splitPythonCode(filePath: string, fileContent: string): Promise<{
    content: string;
    metadata: {
            filePath: string;
...
async function readPythonFilesConcurrently(files: string[]): Promise<{
    ...
    let chunks: Array<{
        content: string; metadata: {
            filePath: string; type: 'function' | 'statements' ; ext: 'js' | 'py' | 'ts'
        }
    }> = [];
    ...
    for (const batch of batches) {
        const readOperations = batch.map(async (filePath) => {
            try {
                ...
                const fileChunks = await splitPythonCode(filePath, content);
...
export async function processPythonFiles(rootDir: string) {
    ...
    console.log("DEBUG: py code chunks", result)
